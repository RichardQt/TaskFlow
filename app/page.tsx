...
function TodoApp() {
	...
	const [sortBy, setSortBy] = React.useState<"priority" | "date" | "created">(
		"priority",
	);
	...
	React.useEffect(() => {
		const checkAndSendReminders = async () => {
			try {
				const res = await fetch("/api/bark/remind", { method: "POST" });
				if (res.ok) {
					const data = await res.json();
					if (data.remindedCount > 0) {
						console.log(`已发送 ${data.remindedCount} 个任务提醒`);
					}
				}
			} catch (error) {
	...
	const handleQuickAdd = (e: React.KeyboardEvent) => {
		if (e.key === "Enter" && quickInput.trim()) {
			addTask({
				...
				priority: "medium",
				projectId:
					activeView !== "today" && activeView !== "all" ? activeView : "inbox",
				recurring: "none",
				dueDate:
					activeView === "today"
		...
		<div className="flex h-screen bg-background">
			...
			<div className="hidden md:flex w-64 border-r bg-card">
				<Sidebar
					...
					barkDeviceCount={barkDevices.filter((d) => d.enabled).length}
					onOpenBarkSettings={() => setShowBarkSettings(true)}
				/>
			</div>
			...
			{/* Bark Settings Dialog */}
			<BarkSettings
				open={showBarkSettings}
				onOpenChange={setShowBarkSettings}
				devices={barkDevices}
				onAddDevice={addBarkDevice}
				onUpdateDevice={updateBarkDevice}
				onDeleteDevice={deleteBarkDevice}
				onTestDevice={testBarkDevice}
			/>
		</div>
	);
}
